...
class TrainerCtc:
    def __init__(self,
        ...
        if not os.path.exists(self.out_model_dir) and self.ddp_local_rank in (-1, 0):
            os.mkdir(self.out_model_dir) # os.makedirs创建目录（即使上级目录不存在），慎用
        ...
        self.tokenizer = CtcTokenizer.from_pretrained(
            self.in_model_dir)
        self.train_ds, self.dev_ds, self.test_ds = self.load_data(fold=self.fold)
    ...
    def load_data(self,fold=0) -> List[DataLoader]:
        ...
        dev_ds = DatasetCTC(
            in_model_dir=self.in_model_dir,
            src_texts=df_dev["source"].tolist(),
            trg_texts=df_dev["target"].tolist(),
            max_seq_len=self.max_seq_len,
        ) 

        # if self.dev_fp is not None:
        #     dev_src_texts, dev_trg_texts, folds = self.load_texts_from_fp(
        #         self.dev_fp, self.n_fold)
        #     dev_ds = DatasetCTC(
        #         in_model_dir=self.in_model_dir,
                ...
                if (step % self.check_val_every_n_steps == 0 or epoch_end_flag) and self.ddp_local_rank in (-1, 0):
                    ...
                    if eval_epoch_f1 >= 0:
                        ...
                        saved_dir = os.path.join(
                            self.out_model_dir, 'fold_'+str(self.fold), metric_str)
                        if self.ddp_local_rank in (-1, 0):
